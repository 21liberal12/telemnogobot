...
start = {
    'mes': 'start',
    'text': '''Привет! Владелец бота: @K0TEH04EK 
    Чат бота: https://t.me/joinchat/O-B3Hxd9P2T87hN3M2q_SQ
    Канал бота: @telemnogo_channel''',
    'markup': [
        ['👁 Смотреть пост', '📲Подписаться на канал'],
        ['💼Для Рекламодателей'],
        ['👨‍⚖️Мой профиль', '💰Кошелек'],
        ['📈Статистика', '🥂Рефералы'],
        ['✅Правила'],
    ]}
# '💰Кошелек','🥂Рефералы'
...
rules = {
    'text': ''' ПРАВИЛА использования бота
	  ...
	  1. ВАЖНО! После пополнения баланса вы автоматически переходите в статус "Рекламодатель" и больше не имеете возможности выводить средства!
	     Попытка нарушения данного пункта приводит к полному обнулению баланса без возврата средств. При повторной попытке - БАН.
	  2. При обнаружении вывода денег на кошелек, с которого уже производилась оплата с любого другого аккаунта - Предупреждение с обнулением баланса. При повторе - БАН. 
	  
	   
3.Запрещено⛔️:
 3.1. Отписываться от канала в течение 7 дней. 
 3.2. Спамить бота повторными командами. 
 3.3. На один кошелек выводить средства с 2 и более аккаунтов.
 3.4. Запрещается смена username.

 За несоблюдение правил баланс пользователя обнуляется. 


4.Рекламадателям запрещено⛔️:
 4.1. Размещать каналы или посты порнографического и  мошеннического содержания. 
 4.2. Заниматься негативным психологическим воздействием, пропагандой наркотиков и терроризма.
 При обнаружении мы будем вынуждены заблокировать пользователя полностью без возврата средств.
 4.3. После оформления заказа убирать права у бота.
 В случае обнаружения ваш заказ удаляется, деньги не возвращаются.
 Права у бота можно убрать после окончания заказа!
 
 5.Любой из пунктов может быть изменен, удален или добавлен администрацией в любой момент без предупреждения пользователей. 
 
 6. Администратор( @K0TEH04EK ) всегда прав.
 
 7.Пользуясь данным ботом, вы автоматически даете свое согласие с данными правилами. 
...
➖➖➖➖➖➖➖➖➖➖➖➖
1️⃣
За каждого приведенного друга в нашего бота вы получаете 0.3р.
Зачисление автоматически.
➖➖➖➖➖➖➖➖➖➖➖➖
2️⃣
...
rules3 = {
    'text': '''Хочешь такого бота? Пиши @R0LLS''',
    'markup': start['markup']
}
...
Минимальная стоимость одного подписчика -  {} руб'''.format(settings.min_post_cost),
    ...
    'error_money': {
        'text': '''🚫 Ошибка! Недостаточно средств на счету''',
        'markup': start['markup']
    },
    'already_in': {
...
⛔️Минимальная стоимость: {} рублей.'''.format(settings.min_post_cost),
    ...
    'error_not_admin': {
        'text': '''🚫 Ошибка! Бот не обнаружен в администраторах канала. Добавьте бота и нажмите кнопку ниже.''',
        'markup': [
            [{'text': 'Проверить', 'data': 'check_admin'}, {'text': '❌ Отклонить', 'data': 'cancel_check_admin'}]]
    },
    'error_low_cost': {
...
💵 Заработано с подписок: *%money_for_views%*р
💵 Заработано с просмотров: *%money_for_post%*р
💵 Заработано с рефералов: *%money_for_refs%*p
...
⛔️Очень важно оставить код в комментарии, иначе платеж не будет засчитан! Свой код обязательно нужно написать владельцу @K0TEH04EK

⚡️После перевода сумма зачислится в течение 1 часа'''.format(settings.number),
        'markup': start['markup']
    },
    'success': {
...
⚡️После перевода сумма зачислится в течение 1 часа'''.format(
            settings.ya_number),
        'markup': start['markup']
    }
}
