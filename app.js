...
const bot			=		new Telegram(
	"987842329:AAGVjZmjYW_pQTG3EDG7hCWgcyB1PNozzEs",
	{ polling: true }
);
...
setInterval(async () => {
	wallet.getOperationHistory({
		...
		res.data.map(async (operation) => {
			...
			if(!operation.comment.startsWith("ms")) return;

			let user = await User.findOne({ id: Number(operation.comment.split("ms")[1]) });
			if(!user) return;
...
const settings		=		{
	pps: 0.15, //цена выдаваемая за подписку
	ppv: 0.02, //цена выдаваемая за просмотр
	ppr: 0.25,
	ref1st: 0.20,
	ref2st: 0.10,
	min_withdraw: 10
...
bot.on("message", async (message) => {
	...
	let { status } = await bot.getChatMember("@MainSubscribers", message.from.id); 
	if(status === 'left') return message.send(`📝 *Для того, чтобы* начать/продолжить работу с ботом, *вы должны быть подписанным* на данные ресурсы и после этой нажать в любую кнопку`, {
		parse_mode: 'Markdown',
		reply_markup: {
			inline_keyboard: [
				[{ text: '📝 Подписаться', url: 'https://t.me/MainSubscribers' }]
			]
		}
	});	
	...
	if(message.user && message.user.menu) {
		if(message.user.menu === "sponsor") {
			...
			bot.sendPhoto(511248254, message.photo[message.photo.length - 1].file_id, {
...
📌 P.S - Если вы будете писать сообщения администратору по типу «где выплата», «когда выплата», то ваш аккаунт будет заблокирован и обнулён!`, {
		...
		if(message.user.menu === "enterCountChannel") {
			...
			if(cost > message.user.balance) return message.send(messages.pr.sub_err_nomoney);
				...
				await channel.save();
				bot.sendMessage("@MainSubscribers", "📃 В боте появилось задание на "+message.text+" подписок", {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🚀 Перейти в бота', url: 'https://t.me/MainSubscribersRobot' }]
                        ]
                    }
                });	
		...
		if(message.user.menu === "enterCountViews") {
			...
			if(cost > message.user.balance) return message.send(messages.pr.view_err_nomoney);
				...
				await post.save();
				await bot.sendMessage("@MainSubscribers", "📃 В боте появилось задание на "+message.text+" просмотров", {
                    reply_markup: {
                        inline_keyboard: [
                            [{ text: '🚀 Перейти в бота', url: 'https://t.me/MainSubscribersRobot' }]
                        ]
	...
	if(message.text === "💵 Заработать") {
		var posts = await Post.countDocuments()
		var channels = await Channel.countDocuments()
		var vsego = channels * 0.15 + posts * 0.02

		return message.send(messages.earn_select + `\n\nВы можете заработать: ${Math.round(vsego * 100.0) / 100.0}р`  +"\n\n➖➖➖➖➖➖\n" + adversite, {
			parse_mode: "HTML",
			reply_markup: {
...
Для того, чтобы получить верификацию вы должны пополнить баланс на любую сумму (можно даже 1 рубль)`, {
	...
	if(message.text === "📥 Пополнить") {
		return message.send(`👛 Кошелёк QIWI: <code></code>
📝 Комментарий к платежу: <code>ms${message.from.id}</code>
...
🔗 Ваша ссылка: https://t.me/MainSubscribersRobot?start=${message.from.id}`, {
			parse_mode: "HTML"
		});
	}

	if(message.text === "📋Правила") {
		return message.send(`❗️Правила использования бота: 
		👁‍🗨Общее положение и требование к пользователям и исполнителям сервиса: 
		
		•1. Исполнитель - пользователь, который выполняет задания обязуется: 
		•1.1. Не отписываться от канала(ов) минимум 7 дней. 
		•1.2. Aккаунт пользователя должен обязательно иметь фото и @username.
		•1.3. Запрещено иметь невнятный юзернейм, пример: @sdofhsdfj
		•1.4 Пополненные деньги, можно потратить только на рекламу раскрутки канала или поста, запрещено вывести.
		•1.5  Запрещены мультиаккаунты
		•1.6  Запрещается писать сообщения администратору по типу «где выплата», «когда выплата». В противном случае, ваш аккаунт будет заблокирован навсегда.
		•1.7  Запрещается любому исполнителю попросить в долг администратции, за попрошайничество, блокировка!
		
		🔺 В случае нарушении одного или нескольких пунктов, ваш профиль будет заблокирован
		
		🔉Требования для заказчика: 
		
		•1. Запрещено добавлять и продвигать каналы которые пропагандируют или связаны с: порнографией/насилием/терроризмом/наркотиками. 
		•1.1 В ходе выполнения заказа нельзя менять username канала и удалять бота с администратора канала и/или менять его права. 
		•1.2 Пополненные деньги, можно потратить только на рекламу раскрутки канала или поста, запрещено вывести.
		
		🔺 В случае нарушения пункта 1/1.1 канал будет удален с раскрутки и/или баланс будет онулирован.`, {
			parse_mode: "HTML"
		});
	}
...
📂 Постов на продвижении: <b>${counters.posts}</b>`, {
	...
	if(message.text === "👁‍🗨Информация") {
		return message.send(`📧 Чат: https://t.me/MainSubscribersChat
		📤 Выплаты:  https://t.me/MainSubscribersOutputs`);
	}

	if(/^(?:~)\s([^]+)/i.test(message.text)) {
		if(message.from.id !== 511248254) return;
...
1		—	100%`, {
		...
		if(message.text === "🔎 Задания") {
			return message.send('Какие заказы вы хотите посмотреть?', {
				reply_markup: {
	...
	if(admins.indexOf(message.user.id) !== -1) {
		...
		if(query.data.startsWith("withdraw")) {
			...
			await wallet.toWallet({
				...
				comment: "@MainSubscribersRobot"
			}, (err, success) => {});

			bot.sendMessage(ticket.owner, "Ваша заявка на вывод была одобрена.");
			bot.sendMessage("@MainSubscribersOutputs", `🤖 <b>Была произведена новая выплата!</b>
👤<b>Пользователь: <a href="tg://user?id=${ticket.owner}">Пользователь</a> </b>	
