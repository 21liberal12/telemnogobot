...
💸 Выведено всего: *{}*p
    ...
    if text == '💰 Пополнение':
        answr = {'text': '''Выберете способ пополнения:''',
                 'markup': [['Киви', 'Другой способ'], ['◀️ Вернуться']]}
        send_message(message, answr)

        return

    if text == 'Другой способ':
        return send_message(message, {'text': 'Если вы хотите пополнить баланс другим способом - напишите [владельцу бота @K0TEH04EK]( @K0TEH04EK )', 'markup': messages.start['markup']}, parse_mode="Markdown")
    if text == '🔖 Мои заказы':
...
🔧🔨Чтобы добавить ваш канал для раскрутки - действуйте по инструкции, указанной выше.''', 'markup': messages.for_subs['markup']})
    ...
    if text == 'Киви':
        ...
        return
    if text == 'YandexMoney (недоступно)':
...
def ymon():
    token = api.Wallet.get_access_token(client_id='103703A821D04244A16FA4554553725282C76EF2E5491E3690FAC425D7665EB4', code=flask.request.args['code'],
                                        redirect_uri='https:///ya_pay')['access_token']
